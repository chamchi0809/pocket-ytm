...
if [ ! -x "$dependency_dir/bin/pocket-ytm-bridge" ] || \
   [ ! -x "$dependency_dir/bin/yt-dlp-runtime/yt-dlp" ] || \
...
rm -rf "$bundle"
install -d "$contents/MacOS" "$contents/Resources/bin" "$contents/Resources/licenses"
...
for dependency in pocket-ytm-bridge ffmpeg ffprobe deno; do
    install -m 755 "$dependency_dir/bin/$dependency" "$contents/Resources/bin/$dependency"
done
ditto "$dependency_dir/bin/yt-dlp-runtime" "$contents/Resources/bin/yt-dlp-runtime"
